...
```text
$co-explore-zh
我没有明确目标。随手选一个具体入口，和我一起开始探索。
入口不需要冷门、有趣、古老或像一个好选题；它只负责让第一步发生。
不要预设最终主题或结论。
```
...
- 我们也试过给 Skill 加固定步数和阶段检查，结果模型开始服务流程而不是材料；这套机制后来被删除。
- 我们还发现，一旦把“陌生、古老、边缘、被遗忘”写成优先级，模型就会反复挑博物、档案、老网站一类“看起来很探索”的入口；所以现在入口不再承担“有趣”的任务。
...
1. **不预设探索节奏。** 不设固定步数、固定检查点或阶段产出；有新材料就继续，路线枯竭就换路。
2. **入口只负责让第一步发生。** 没有用户入口时，随手选一个具体对象或页面即可，不要为了“探索感”刻意挑冷门、古老、猎奇或有故事的起点。
3. **下一跳来自当前材料。** 不靠纯随机，也不靠预设关键词树。
4. **延迟总结。** 探索阶段先看、先走、先记录，不急着形成“题”。
5. **人和 AI 互相纠偏。** 谁发现对方开始重复，谁就打断路线。
6. **追“为什么”，但用它找材料。** 为什么会这样、为什么是他、为什么存在、为什么是这种形式——这些问题用来打开下一跳，不用来原地脑补解释。
7. **结束以后再回看。** 由用户意愿和探索状态决定何时停，最后才整理真正发生了什么变化。
